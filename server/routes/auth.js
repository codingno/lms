...
router.get("/auth/info", (req, res) => {
  if (req.user) res.ok({ user: req.user });
  else res.ok(null);
});
  ...
  async.waterfall([
    function (cb) {
      userAuthService.findByEmail(req.body.email, function (err, data) {
        ...
        else if (data.length > 0) cb(null, data[0]);
        else return res.badRequest({ err: "email unregistered!" });
      });
    },
    function (data, cb) {
    ...
    function (data, cb) {
      sendEmailForgotPass(
        ...
        function (err, data) {
          if (err) {
            console.log("error send email forgot password", { err });
            res.badRequest("failed to send email");
          } else res.ok("success to send email");
        }
      );
    },
  ]);
...
router.get("/checkResetToken/:token", function (req, res) {
  userAuthService.findByPasswordToken(req.params.token, function (err, data) {
    if (err) res.badRequest("invalid token");
    else res.ok("valid token");
  });
});
...
router.post("/resetPassword", function (req, res) {
  userAuthService.resetPassword(req.body, function (err, data) {
    ...
    } else res.ok("success to reset password");
  });
});
